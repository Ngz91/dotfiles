...
function _DJANGO_CREATEAPP()
	...
	local django_createapp =
		Terminal:new({ cmd = string.format("cd * && python3 manage.py createapp %s", app_name), hidden = true })
	django_createapp:toggle()
end

local django_shell = Terminal:new({ cmd = "cd * && python3 manage.py shell", hidden = true, direction = "vertical" })
local django_db_shell =
	Terminal:new({ cmd = "cd * && python3 manage.py dbshell", hidden = true, direction = "vertical" })
...
local django_runserver =
	Terminal:new({ cmd = "cd * && python3 manage.py runserver", hidden = true, direction = "vertical" })
